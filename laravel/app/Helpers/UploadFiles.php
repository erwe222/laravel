...
class UploadFiles {
    ...
    public function __construct($array,$dir_name) {
        $this->array     = $array;
	...
	if (@is_writable($this->dir_name) === false) {
        ...
        if(count($this->array) > 0){
            foreach($this->array as $key=>$v){
                ...
                $this->array[$key]['save_dir'] = $this->dir_name.'/'.$this->array[$key]['new_name'];
            }
        }
    ...
    public function upload(){
        ...
        foreach ($this->array as $key => $value) {
            if (move_uploaded_file($value['tmp_name'], $this->dir_name) === false) {
